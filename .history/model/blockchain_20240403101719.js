...
var blockchain = function blockchain() {
  ...
  function checkChain() {
    ...
    for (let i = 1; i <= self.chain.length; i++) {
      ...
      //Skip validation for the genesis block(i=1)
      if (i > 1) {
        console.log("second test starts !!");
        console.log("previous block not checked for genesis block");

        previousBlock = self.chain[i - 2];
        console.log(`previousBlock for ${i}: `, previousBlock);
        console.log(
          `previousBlock transaction for ${i}: `,
          previousBlock.transaction[0]
        );
        previousBlockFirstTransaction = previousBlock.transaction[0];
        ...
        if (
          ...
          validator.calculateHash(previousBlockFirstTransaction)
        ) {
          console.log("has failed in second test");
